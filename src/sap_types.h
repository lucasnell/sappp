...
// Maybe make it 3d array: max_t, n_lines, n_patches


// Summarize a patch's populations for all lines at a given time
//' @export PatchSummary
struct PatchSummary {

    vector<double> aphids;          // density of unparasitized aphids
    vector<double> parasit;         // density of parasitized, but alive, aphids
    vector<double> mummies;         // density of mummies
    vector<double> wasps;           // density of adult wasps
    
    PatchSummary() : aphids(0), parasit(0), mummies(0), wasps(0) {};
    PatchSummary(vector<double> aphids_, vector<double> parasit_, 
                 vector<double> mummies_, vector<double> wasps_) 
        : aphids(aphids_), parasit(parasit_), mummies(mummies_), wasps(wasps_) {};
    
    void append(double aphids_, double parasit_, double mummies_, double wasps_) {
        aphids.push_back(aphids_);
        parasit.push_back(parasit_);
...
PatchSummary OnePatch::get_summary() {
...
public:
    ...
    vector<PatchSummary> simulate(uint max_t, uint rng_seed);
...
vector<PatchSummary> SimPatches::simulate(uint max_t, uint rng_seed) {
    sitmo::prng_engine eng(rng_seed);
    ...
    while (t < max_t) {
        ...
        dispersal();
        // PatchSummary summ = p.get_summary()
        t++;
    }
